...
bool config_is_white_to_move(config_t *config) {
    return config->active_color == 'w' ? true : false;
}

void config_set_white_to_move(config_t *config, bool white_to_move) {
    config->active_color = white_to_move ? 'w' : 'b';
}
